...
module.exports = {
    name: 'even',
    description: 'Bet that a roll will be even.',
    args: true,
    argCount: 2,
    ...
    async execute(message, args) {
        ...
        switch(betValidity){
            case currencyHandler.BetValidityEnum.VALID_BET:
                ...
                let result = null;
                const checkEven = this.name === 'even';
                if((checkEven?rollValue%2===0:rollValue%2===1)){
                ...
                resultsEmbed.addField(`${this.name}x${this.multiplier} Dicing`, `Rolled **${rollValue}**(**${(rollValue%2===0?"EVEN":"ODD")}**) out of 100. You **${(wagerResult > wagerAmount?"won":"lost")} ${currencyResultString}**.`)
                return message.channel.send(resultsEmbed).catch(console.error);
                break;
