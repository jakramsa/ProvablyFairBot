...
module.exports = {
    name: 'flip',
    description: 'Bet that a coin will fall heads or tails.',
    aliases: ['coin'],
    args: true,
    argCount: 3,
    usage(){ return `${prefix}${this.name} [currency] [amount[multiplier]] [heads or tails]`; },
    cooldown: 0,
    guildOnly: true,
    multiplier: 1.9,
    async execute(message, args) {
        ...
        switch(betValidity){
            case currencyHandler.BetValidityEnum.VALID_BET:
                if(args[2][0] !== 'h' && args[2][0] !== 't'){
                    resultsEmbed.addField("Error", "You must specify either heads or tails.")
                    .setColor(embedColors.error);
                    return message.channel.send(resultsEmbed).catch(console.error);
                }
                const guessedHeads = (args[2][0] === 'h');
                ...
                let result = null;
                if((guessedHeads?(rollValue < 51):(rollValue > 50))){
                ...
                resultsEmbed.addField(`${this.name}x${this.multiplier} Coin Game`, `The coin landed **${(rollValue<51?"HEADS":"TAILS")}**. You **${(wagerResult > wagerAmount?"won":"lost")} ${currencyResultString}**.`)
                return message.channel.send(resultsEmbed).catch(console.error);
                break;
